...
export async function enviarReporteCliente(opts: {
  ...
  if (opts.cliente.email && transport) {
    await transport.sendMail({
      from: env.smtp.from,
      to: opts.cliente.email,
      subject: `Tu reporte Canela Coach® — ${opts.cliente.nombre}`,
      html: `
        <div style="font-family:sans-serif;background:#F4F7FA;padding:24px">
          <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden">
            ...
            <div style="padding:24px;color:#0B1220">
              <p>Hola <strong>${opts.cliente.nombre}</strong>,</p>
              <p>Tu reporte de evaluación física está listo.</p>
              <p><a href="${opts.pdfUrl}" style="display:inline-block;background:#2E9BE6;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none">Descargar reporte PDF</a></p>
              <p style="color:#9AA5B1;font-size:13px">Código: ${opts.cliente.codigoCliente}</p>
            </div>
          </div>
        </div>
      `,
  ...
  if (opts.cliente.telefono && env.whatsapp.token && env.whatsapp.phoneId) {
    try {
      await fetch(
        `https://graph.facebook.com/v19.0/${env.whatsapp.phoneId}/messages`,
        {
          ...
          body: JSON.stringify({
            ...
            type: 'text',
            text: {
              body: `Hola ${opts.cliente.nombre}, tu reporte Canela Coach® está listo: ${opts.pdfUrl}`,
            },
          }),
        }
      );
